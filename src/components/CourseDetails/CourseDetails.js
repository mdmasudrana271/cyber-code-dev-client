...
const CourseDetails = () => {
  ...
  const { image, title, rating, price, description, instructor, enroll, _id } =
    course;
  console.log(course);
  ...
  return (
    <div
      ...
      ref={ref}
      className="flex flex-col  mx-auto my-20 space-y-6 bg-white overflow-hidden rounded-lg shadow-md text-black"
    >
      <div>
        <div className="flex justify-center items-center px-5">
          ...
          <Pdf targetRef={ref} filename="course details.pdf">
            {({ toPdf }) => (
              <button className="px-5 text-xl" onClick={toPdf}>
        ...
        <img
          ...
          alt=""
        />
        <p className="text-lg text-black px-5">{description}</p>
      </div>
      <div className="flex flex-wrap justify-between px-5">
        <div className="space-x-2">
          <p className="flex items-center text-xl">
            <FaStar className="text-amber-400 mx-1"></FaStar>{" "}
            <span className="text-black">{rating}</span>
          </p>
          <p className="my-3 text-xl">price: {price}</p>
        </div>
        <div className="flex justify-end space-x-2 text-sm dark:text-gray-400">
          <p className="text-black text-xl flex items-center gap-2">
            Enrolled: {enroll} <FaUserFriends />
          </p>
        </div>
      </div>
      ...
      <div className="flex justify-end space-x-2 text-sm dark:text-gray-400 p-5">
        <Link to={`/coursedetails/${_id}`}>
          <button className="btn btn-success text-bold hover:bg-indigo-600 hover:text-white">Get premium access.</button>
        </Link>
      </div>
    </div>
  );
